    ...
    <section className="min-h-screen bg-white px-1 md:px-6 md:px-16 py-28">
      ...
      <div className="columns-2 md:columns-3 gap-0.5 space-y-0.5">
        ...
        {photos.map((photo, index) => (
          <div
            key={index}
            className="mb-[4px] break-inside-avoid overflow-hidden rounded-md group"
          >
            <img
              src={photo}
              alt={`newborn-${index}`}
              className="w-full object-cover transition duration-700 ease-out group-hover:scale-105 group-hover:brightness-110"
            />
